...
const PollView = () => {
  ...
  useEffect(() => {
    const loadPoll = async () => {
      console.log('[PollView] Loading poll with linkId:', linkId);
      setLoading(true);

      const pollData = await getPollByLink(linkId);
      console.log('[PollView] Poll data received:', pollData);
      ...
      if (pollData) {
        ...
        console.error('[PollView] Poll not found for linkId:', linkId);
      }
    ...
    return (
      <div className="poll-view-container">
        <div className="poll-loading">
          ...
          <h2>Loading Poll...</h2>
          <p>Please wait while we fetch the poll data.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="poll-view-container">
        <div className="poll-not-found">
          ...
          <p className="debug-info">Link ID: {linkId}</p>
          <p className="debug-help">Please check the link and try again, or contact the poll creator.</p>
        </div>
      </div>
    );
  }
